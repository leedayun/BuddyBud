...
public class DataManager {
    ...
    private DataManager() {
        ...
        comment3.add(new CommentData(1, R.drawable.test, "seven77", "23/11/14 17:34",
        ...
        comment3.add(new CommentData(3,R.drawable.test, "happy486", "23/11/14 17:46",
        ...
        comment3.add(new CommentData(6,R.drawable.test, "유저F", "23/11/14 17:36",
        ...
        comment2.add(new CommentData(2,R.drawable.test, "머라하지", "23/11/14 17:45",
                "저도용 근데 라켓 없어도 되나요?", "Me too, but can I do it without a racket?", 0, 2, 1,"스꾸"));
        comment2.add(new CommentData(3,R.drawable.test, "Joy", "23/11/14 17:50",
                "as far as I know, Joa Tennis has racket rental service", "as far as I know, Joa Tennis has racket rental service", 5, 0, 1,"머라하지"));
        comment2.add(new CommentData(4,R.drawable.test, "软锥", "23/11/14 17:41",
                "这是我第一次打网球，可以吗？", "This is my first time playing tennis, is it okay?", 1, 1, 4,null));
        comment2.add(new CommentData(5,R.drawable.test, "milililili", "23/11/14 17:41",
                "当然！", "Certainly!", 2, 0, 4,"软锥"));

        comment1.add(new CommentData(1,R.drawable.test, "软锥", "23/11/14 18:34",
        ...
        feedList.add(new FeedData(2, R.drawable.icon_account, "mililili", "23/11/14 17:13", "Anyone who wants to play tennis?",
    ...
    // 댓글 / 대댓글 추가 - 피드 자세히 보기의 댓글 작성 부분에서 사용
    public void addCommentToFeed(int feedId, CommentData comment) {
        for (FeedData feed : feedList) {
            if (feed.getId() == feedId) {
                if (comment.getReplyToCommentId() == 0) {
                    feed.getComments().add(comment); // 댓글인 경우 마지막에 추가
                } else {
                    // 대댓글인 경우 올바른 위치에 삽입
                    int insertIndex = findInsertIndexForReply(feed.getComments(), comment.getReplyToCommentId());
                    feed.getComments().add(insertIndex, comment);
                }
                break;
            }
        }
    ...
    public List<FeedData> filterFeeds(String query) {
        ...
        for (FeedData feed : feedList) {
            if (feed.getTitle().toLowerCase().contains(query.toLowerCase()) || feed.getContent().toLowerCase().contains(query.toLowerCase()) ||
                    feed.getTranslateTitle().toLowerCase().contains(query.toLowerCase()) ||feed.getTranslateContent().toLowerCase().contains(query.toLowerCase()) ) {
